...
import { recordBookProgress } from "../progression/xpStats";
import {
...
const handleLocation = (book: Book, rendition: any) => {
  let position = rendition.getPosition();
  recordBookProgress(book, position);
};
...
export const bindHtmlEvent = (
  ...
  doc.addEventListener(
    ...
    async (event) => {
      if (lock) return;
      lock = true;
      await arrowKeys(rendition, event, readerMode, format, book.key);
      handleLocation(book, rendition);
...
export const htmlMouseEvent = (
  ...
  rendition.on("rendered", () => {
    ...
    if (!iframe) return;
    iframe?.focus();
    let docs = getIframeDoc(format, book.key);
    for (let i = 0; i < docs.length; i++) {
      let doc = docs[i];
      if (!doc) continue;
      bindHtmlEvent(
