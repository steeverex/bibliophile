...
class Header extends React.Component<HeaderProps, HeaderState> {
  ...
  private readingFinishedHandler: ((event: any, config: any) => void) | null =
    null;
  private bookOpenedHandler: (() => void) | null = null;
  ...
  async componentDidMount() {
    this.atmosphereEnabledHandler = () => {
      const isAtmosphereEnabled =
        ConfigService.getReaderConfig("atmosphereEnabled") === "yes";
      this.setState({ isAtmosphereEnabled });
    };
    ...
    if (isElectron) {
      ...
      this.bookOpenedHandler = () => {
        audioManager.play().catch((err) => {
          console.warn("[AudioManager] play error:", err);
        });
      };
      ipcRenderer.on("book-opened", this.bookOpenedHandler);
      ipcRenderer.on(
        "open-book-from-link",
  ...
  componentWillUnmount() {
    ...
    if (isElectron && this.bookOpenedHandler) {
      const { ipcRenderer } = window.require("electron");
      ipcRenderer.removeListener("book-opened", this.bookOpenedHandler);
      this.bookOpenedHandler = null;
    }
  ...
  handleAtmosphereToggle = async () => {
    const next = ConfigService.getReaderConfig("atmosphereEnabled") !== "yes";
    ConfigService.setReaderConfig("atmosphereEnabled", next ? "yes" : "no");
    this.setState({ isAtmosphereEnabled: next });
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className={
              "setting-icon-container atmosphere-toggle " +
              (this.state.isAtmosphereEnabled
                ? "atmosphere-toggle-on"
                : "atmosphere-toggle-off")
            }
            onClick={() => {
              this.handleAtmosphereToggle().catch((error) => {
                console.warn("[AudioManager] toolbar toggle failed:", error);
              });
            }}
            style={{ marginTop: "2px" }}
          >
            <span
              ...
              data-tooltip-content={
                this.state.isAtmosphereEnabled
                  ? this.props.t("Disable background music")
                  : this.props.t("Enable background music")
              }
              data-tooltip-place="left"
            >
              <span className="icon-audio setting-icon atmosphere-toggle-icon" />
            </span>
          </div>
        </div>
