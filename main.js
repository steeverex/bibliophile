...
// errors continue to surface as uncaught exceptions.
...
const createMainWin = () => {
  ...
  ipcMain.handle("discord-rpc-update", async (event, config) => {
    ...
    try {
      ...
      await discordRPCClient.setActivity({
        ...
        largeImageText: "Bibliophile",
        startTimestamp: Date.now(),
        instance: false,
        buttons: [
          {
            label: "Get Bibliophile",
  ...
  ipcMain.handle("open-book", (event, config) => {
    ...
    if (isPreventSleep === "yes") {
      ...
      log.info("Power-save blocker started:", powerSaveBlocker.isStarted(id));
    }
    if (readerWindow) {
    ...
    // Notify the main window renderer so it can start atmosphere audio
    if (mainWin && !mainWin.isDestroyed()) {
      mainWin.webContents.send("book-opened");
    }
